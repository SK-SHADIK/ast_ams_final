...
class AssetTransactionsController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('invoice', __('Invoice'))->display(function ($invoice) {
            $url = asset('upload/' . $invoice);        
            return "<a href='$url' target='_blank'>$invoice</a>";
        });
    ...
    protected function form()
    {
        ...
        $form->file('invoice',__ ('Invoice'))->move('pdfFiles');
        ...
        $form->saving(function (Form $form) {
            try{
                   $file = $form->invoice;
                    ...
                    $path = $file->storeAs('pdfFiles', $filename, 'public');
            } catch (Exception $ex) {
                    Log::error(_FILE_ . ' || Line ' . _LINE_ . ' || ' . $ex->getMessage() . ' || ' . $ex->getCode());
            }
            });
